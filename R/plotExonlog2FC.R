plotExonlog2FC <- function(results.Data=NULL, out.Dir=NULL, FDR.cutoff=0.05){
    ...
    if (is.null(out.Dir) == TRUE){
        stop(call="plotExClustResults requires a direcotry to write images to, but was not specified. Please specify the full file path to a folder to which exon log2FC plots may be written.")
    }else{
    ...
    for (i in 2:nrow(sig.Results)){
        ...
        if ((sig.Results$IDs[i] != sig.Results$IDs[(i-1)]) == TRUE | i == nrow(sig.Results)){
            ...
            #png(filename=paste(out.Dir,"/",TargetGene,"_",TargetName,".png",sep=""),width=6,height=4, units="in",res = 300)
            bitmap(file=paste(out.Dir,"/",TargetGene,"_",TargetName,".png",sep=""), type="png16m",width=6,height=4, units="in",res = 400, taa=4,gaa=4)
            par(ps = 12, cex = 1, cex.main = 1,mar=(c(5, 5, 4, 2)+0.2))
            plot(c(0,MaxX), c(-(Maxlog2FC),Maxlog2FC) , type= "n", ylab = "log2 fold change (log2FC)", xlab = "nucleotide position (number of base pairs)",main = paste(TargetName,"   ",TargetGene,"    ","FDR=",TargetFDR,sep=""),cex.main=0.9)
            ...
            for (j in 1:nrow(TempData)){
                rect(ExonStart, TempData$log2FC[j]-BH, ExonStart + TempData$Length[j] , TempData$log2FC[j]+BH, col = "red")
                ErrorBar((ExonStart + (TempData$Length[j] / 2)),(TempData$log2FC[j]-sqrt(TempData$log2Var[j])), (TempData$log2FC[j]+sqrt(TempData$log2Var[j])),MaxX,TempData$Length[j])
